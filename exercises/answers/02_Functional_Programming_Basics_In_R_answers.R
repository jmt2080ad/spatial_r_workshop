...
data$newCol1     <- data$measurement + 3
data[,"newCol2"] <- data$measurement / 2
data["newCol3"]  <- data$measurement * data$measurement 

## Create a new column using `ifelse`:
## If type is "planned", then measurement is set to NA, else type is
## unchanged.
## Tip: See `?is.na()`.
data$newCol1 <- ifelse(data$type == "planned", NA, data$type)
...
mapply(pointFun, data$x, data$y, SIMPLIFY = F)
